...
class App(CTk):
    ...
    def search_word(self):
        try:
            ...
            definitions = CTkTextbox(self, width=260, height=400, font=('Segoe UI', 16), scrollbar_button_color='darkgreen', scrollbar_button_hover_color='black')
            ...
            def hear():
                try:
                    ...
                    with open(f'{json[0]["word"]}.mp3', 'wb') as f:
                        f.write(r.content)
                    playsound.playsound(f'{Path().cwd() / f"""{json[0]["word"]}.mp3"""}', block=False)
            ...
            show_api_response = CTkButton(self, width=100, height=30, text='Show API response', command=lambda: webbrowser.open(url), font=('Segoe UI', 16, 'bold'))
